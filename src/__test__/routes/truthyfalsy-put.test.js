...
describe('PUT /api/v1/truthyfalsy', () => {
  ...
  describe('Valid', () => {
    it('should return a 204 status (CHANGED)', () => {
      return mocks.truthyfalsy.createOne()
        .then(mock => {
          mock.truthyfalsy.questions = [];
          return superagent.put(`${GAME_ENDPOINT}/${mock.truthyfalsy._id}`)
            .set('Authorization', `Bearer ${mock.profile.user.token}`)
            .send(mock.truthyfalsy);
        })
        .then(res => {
  ...
  describe('Invalid route', () => {
    it('should respond with a 404 not found', () => {
      return mocks.truthyfalsy.createOne()
      .then(mock => {
        return superagent.put(`${GAME_ENDPOINT}/${mock.truthyfalsy._id}nope`)
          .set('Authorization', `Bearer ${mock.profile.user.token}`)
          .send();
      })
      .then(response => console.log('error did not throw: res:', response))
